...
一个面向生产的 **FastAPI** 服务，用于**高并发**地提供 Binance 合约 K 线数据读取。具备 **自动同步交易对**、**历史回补**、**本地存储与缓存**、**条件请求（ETag/304）**、**Prometheus 指标** 等工程化能力。
...
- **交易对自动同步**：后台定时从 `/fapi/v1/exchangeInfo` 拉取 USDT 永续合约，保留 `contractType=PERPETUAL`、`status=TRADING`，并根据 `deliveryDate` 预剔除将/已下架；新增/下架自动生效。  
- **K 线抓取与历史回补**：支持 `symbol / interval / startTime / endTime / limit` 拉取，带 **抖动（jitter）** 与 **重试退避**。  
- **高并发读取与弱缓存**：
  - L1 内存缓存 **O(1) LRU + TTL**，热门区间直出 `bytes`。  
  - HTTP 层自动附带 **ETag** 与 `Cache-Control`；客户端带 `If-None-Match` 命中返回 **304**。  
- **本地存储（默认 SQLite）**：WAL + `busy_timeout`，按周期分表，主键 `(symbol, open_time)`；为 `is_final=1` 查询提供 **部分索引**。可扩展到 **Postgres**。  
- **可观测性**：暴露 **`/metrics`**（Prometheus），以及 **`/v1/health`** 健康检查。
...
### 1) `GET /fapi/v1/klines` — 查询 K 线

返回与 Binance 兼容的二维数组，服务器自动添加 ETag 与 Cache-Control。
...
| `interval` | string | 是 | 周期：`1m,3m,5m,15m,1h,4h,1d` |
| `limit` | int | 否 | 最大条数，默认 500 |
| `startTime` | int(ms) | 否 | 开始时间戳（含） |
| `endTime` | int(ms) | 否 | 结束时间戳（通常不含） |
| `only_final` | bool | 否 | 默认 `true`，只返回“已收盘”K 线 |
...
### 2) `POST /v1/admin/symbols/refresh` — 手动刷新交易对
> 建议仅内网或加鉴权。
...
`.env` 放在**运行目录**下；支持 **逗号 CSV** 或 **JSON 列表** 写法。
...
> 说明：开启 `AUTO_SYNC_SYMBOLS=true` 后，运行时使用**后台同步集合**；`.env` 的 `SYMBOLS` 仅作冷启动兜底。抓取/回补循环应从 `app.state.symbol_registry.get_all()` 读取当前生效交易对集合。
...
# 入口可能是 main.py 或 app/main.py（两种都试一下）
uvicorn main:app --host 0.0.0.0 --port 8000
# 或
# uvicorn app.main:app --host 0.0.0.0 --port 8000
```

### Debian 12 裸机部署（不使用 Docker）
...
3. **systemd 自启**
   ```ini
   # /etc/systemd/system/biance.service
   [Unit]
...
4. **Nginx 反代（推荐）**
   ```nginx
   # /etc/nginx/sites-available/biance.conf
   server {
       listen 80;
       server_name _;
...
- **入口导入失败**：先进入 `biance-main/` 再运行；入口尝试 `main:app` 或 `app.main:app`。
